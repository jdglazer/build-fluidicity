...
#   1. create docker image: docker build --tag build_fluidicity:0.0.1 .
#   2. create and run docker container from image docker: docker run -dt --name bfj_container build_fluidicity:0.0.1
...
#                             PYTHONPATH=/tmp/src pytest --cov=build_fluidicity --cov-report=html && \
